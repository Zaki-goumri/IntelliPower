...
import PowerOptimization from "@/components/power-optimization"
import HVACEfficiency from "@/components/hvac-efficiency"
import EnergyComparison from "@/components/energy-comparison"
import FloorSelector from "@/components/floor-selector"
import useGetData from "./useGetData"
import { fetchDashboardTemperatureData, fetchEnergyConsumationData } from "./dashboard.service"
...
export default function DashboardPage() {
  ...
  const [selectedPeriodOpt, setSelectedPeriodOpt] = useState<Period>("DAY")
  const [selectedPeriodHVAC, setSelectedPeriodHVAC] = useState<Period>("DAY")
  const [selectedPeriodComp, setSelectedPeriodComp] = useState<Period>("DAY")
  const [temperatureData, setTemperatureData] = useState<any[]>([])
  const [energyConsumption, setEnergyConsumption] = useState<any[]>([])

  const [areas, setAreas] = useState<{ value: string; label: string; type: string; id: string }[] | undefined>()
  ...
  useEffect(() => {
    ...
    loadTemperatureData()
    loadEnergyData()
  }, [selectedFloorId, selectedFloorData, selectedPeriodTemp, selectedPeriodEnrg])
  ...
  return (
    <main className="flex min-h-screen flex-col p-6 w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <h1 className="text-3xl font-bold">Intelligent Power & Security System</h1>
      ...
      {selectedFloorData ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <HVACEfficiency period={selectedPeriodHVAC} onChangePeriod={setSelectedPeriodHVAC} />

          <PowerOptimization
            data={energyConsumption}
            period={selectedPeriodOpt}
            onChangePeriod={setSelectedPeriodOpt}
          />


          <EnergyComparison period={selectedPeriodComp} onChangePeriod={setSelectedPeriodComp} />
        </div>
      ) : (
